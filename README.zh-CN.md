...
<details>
<summary>架构图提示词</summary>

```text
为开源项目 "harness-engineering" 创建一张 16:9 系统架构图。
它是一个 Agent Skill 包，把 AI coding 工作转化为受约束、可验证、证据驱动的交付系统。

使用成熟工程架构图风格：系统蓝图、接近扁平矢量的位图、清晰线条、克制专业配色、浅色背景、细连接线、轻微层次感、信息密度高但保持可读。

采用中心内核加交付闭环的架构：
- 中心："SKILL.md Harness 内核"，包含 "启动审计"、"工作流路由"、"重设计模式"
- 左侧输入："人类意图"、"仓库状态"
- 外层四个工程层：
  1. "上下文系统"："AGENTS.md"、"文档地图"、"知识目录"、"参考资料"
  2. "执行 Playbooks"："功能开发"、"Bugfix"、"重构"、"长期构建"、"证据交付"
  3. "质量闸门"："测试"、"Lint / CI"、"审计脚本"、"评估 Rubric"
  4. "记忆工件"："进度跟踪"、"Sprint Contract"、"交接文档"、"Git 提交"
- 右侧交付闭环："计划 → 构建 → 评估 → 证据 → 交接"
- 输出："可验证交付"、"可复用知识"、"受控熵增"、"降低 Agent 漂移"

页脚："人设计约束 • Agent 执行 • 仓库记录事实"
图面要体现约束轨道、反馈信号、工件存储、验证闸门和记忆闭环。避免通用卡片、装饰光斑、logo、水印和过小文字。
```
